...
public class FileStorageService {
    public List<FileContentDTO> storeFiles(MultipartFile[] files) {
        return Arrays.stream(files)
                .map(this::validateAndStore)
                .collect(Collectors.toList());
    }

    private FileContentDTO validateAndStore(MultipartFile file) {
        validateFile(file);
        try {
            return new FileContentDTO(file.getOriginalFilename(), file.getBytes());
        } catch (Exception e) {
